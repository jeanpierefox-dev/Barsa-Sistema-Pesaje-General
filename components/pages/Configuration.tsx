...
import { Printer, Save, Check, AlertTriangle, Bluetooth, Link2, MonitorCheck, Database } from 'lucide-react';
...
const Configuration: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('¡ADVERTENCIA CRÍTICA!\n\nEsto borrará TODOS los datos (usuarios, lotes, ventas) y restablecerá la aplicación a su estado original.\n\n¿Estás seguro?')) {
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-6">
        ...
        {user?.role === UserRole.ADMIN && (
            <div className="border-t border-gray-200 pt-6">
                <h3 className="font-bold text-lg text-gray-900 mb-4 flex items-center"><Database className="mr-2"/> Conexión a Base de Datos (Firebase)</h3>
                ...
                <p className="text-xs text-gray-500 mt-2">Ingrese las credenciales de su proyecto Firebase para sincronizar datos en la nube.</p>
            </div>
        )}
        ...
        <div className="flex justify-between items-center pt-8 mt-8 border-t border-gray-200">
          <button 
            onClick={handleReset}
            className="flex items-center text-red-600 hover:text-red-800 px-4 py-2 rounded-lg border border-red-200 hover:bg-red-50 transition-colors font-bold"
          >
